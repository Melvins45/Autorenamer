...
if __name__ == "__main__":
    ...
    window.__setattr__("categoryGroups", [])
    window.__setattr__("categoryLayouts", [])
    window.__setattr__("categoryTryers", [])
    ...
    def showFiles(_files_names: list[str], _files_sorted: list[list[dict]]) :
        ...
        """
        # return
        window.categoriesNamesInputs = _files_names.copy()
        window.categoryInputs = [ [ j["final"] for j in i ] for i in _files_sorted ]
        window.categoryGroups = [ gf.load_py("renamer") for i in _files_names ]
        window.categoryTryers = [ [ gf.load_py("renamer") for j in i ] for i in _files_sorted ]
        # window.categoryLayouts = [ QVBoxLayout() for i in _files_names ]
        # [ window.__getattribute__(f"categoryLayout{i}").setObjectName(f"categoryLayout{i}") for i in range(len(_files_names)) ]
        gf.clear_layout(window.m_ui.scrollLayout)
        for i in range(len(_files_names)) :
            # window.categoryGroups.append(gf.load_py("renamer"))
            # window.categoryTryers.append([])
            # window.categoryGroups[i].m_ui.category.setText(_files_names[i])
            # window.categoryLayouts.append(QVBoxLayout())
            ...
            for j in range(len(_files_sorted[i])) :
                # window.categoryTryers[i].append
                # window.__setattr__(f"category{i}-{j}", gf.load_py("renamer"))
                if j%2 == 0 :
                    window.categoryTryers[i][j].m_ui.evenOldName.setText(_files_sorted[i][j]['original'])
                    window.categoryTryers[i][j].m_ui.evenNewName.setText(_files_sorted[i][j]['final'])
                    window.categoryTryers[i][j].m_ui.evenNewName.textChanged.connect(lambda _, i=i, j=j :   set_in_2D_list("categoryInputs", i, j, _)) 
                    window.__getattribute__(f"categoryLayout{i}").addWidget(window.categoryTryers[i][j].m_ui.evenContainer) 
                else :
                    window.categoryTryers[i][j].m_ui.oddOldName.setText(_files_sorted[i][j]['original'])
                    window.categoryTryers[i][j].m_ui.oddNewName.setText(_files_sorted[i][j]['final'])
                    window.categoryTryers[i][j].m_ui.oddNewName.textChanged.connect(lambda _, i=i, j=j :   set_in_2D_list("categoryInputs", i, j, _)) 
                    window.__getattribute__(f"categoryLayout{i}").addWidget(window.categoryTryers[i][j].m_ui.oddContainer) 
            window.categoryGroups[i].m_ui.categoryLayout.addLayout(window.__getattribute__(f"categoryLayout{i}"))
            window.categoryGroups[i].m_ui.category.setText(_files_names[i])
            window.categoryGroups[i].m_ui.category.textChanged.connect(lambda _, i=i : set_in_list("categoriesNamesInputs", i, _)) 
            window.categoryGroups[i].m_ui.refresh.clicked.connect(lambda _=0, i=i : refresh_data(i))
            window.m_ui.scrollLayout.addWidget(window.categoryGroups[i].m_ui.categoryGroup)
    ...
    def refresh_data(_index : int) :
        ...
        if window.filesNames[_index] != window.categoriesNamesInputs[_index] :
            categoryObject = gf.get_episode_object(window.categoriesNamesInputs[_index])
